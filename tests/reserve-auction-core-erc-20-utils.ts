...
export function createReserveAuctionCoreErc20RoyaltyPayoutEvent(
  tokenContract: Address,
  tokenId: BigInt,
  recipient: Address,
  amount: BigInt
): ReserveAuctionCoreErc20RoyaltyPayout {
  let reserveAuctionCoreErc20RoyaltyPayoutEvent = changetype<
    ReserveAuctionCoreErc20RoyaltyPayout
  >(newMockEvent())

  reserveAuctionCoreErc20RoyaltyPayoutEvent.parameters = new Array()

  reserveAuctionCoreErc20RoyaltyPayoutEvent.parameters.push(
    new ethereum.EventParam(
      "tokenContract",
      ethereum.Value.fromAddress(tokenContract)
    )
  ...
  return reserveAuctionCoreErc20RoyaltyPayoutEvent
}
