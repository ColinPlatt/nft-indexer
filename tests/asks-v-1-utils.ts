...
export function createAsksV1ExchangeExecutedEvent(
  userA: Address,
  userB: Address,
  a: ethereum.Tuple,
  b: ethereum.Tuple
): AsksV1ExchangeExecuted {
  let asksV1ExchangeExecutedEvent = changetype<AsksV1ExchangeExecuted>(
    newMockEvent()
  )

  asksV1ExchangeExecutedEvent.parameters = new Array()

  asksV1ExchangeExecutedEvent.parameters.push(
  ...
  return asksV1ExchangeExecutedEvent
}

export function createAsksV1RoyaltyPayoutEvent(
  tokenContract: Address,
  tokenId: BigInt,
  recipient: Address,
  amount: BigInt
): AsksV1RoyaltyPayout {
  let asksV1RoyaltyPayoutEvent = changetype<AsksV1RoyaltyPayout>(newMockEvent())

  asksV1RoyaltyPayoutEvent.parameters = new Array()

  asksV1RoyaltyPayoutEvent.parameters.push(
    new ethereum.EventParam(
      "tokenContract",
      ethereum.Value.fromAddress(tokenContract)
    )
  ...
  return asksV1RoyaltyPayoutEvent
}
